...
namespace RRTypes
{
    ...
    public static class KPT_v09Utils
    {
		...
		public static TEntitySpatial KPT09OKSEsToEs(string Definition, tEntitySpatialOKSOut ES)
		{
			...
			for (int i = 0; i <= ES.SpatialElement.Count - 1; i++)
			{
				...
				if (ES.SpatialElement[i].SpelementUnit[0].TypeUnit == tSpelementUnitZUOutTypeUnit.Окружность)
				{
				...
				if (ES.SpatialElement[i].SpelementUnit[0].TypeUnit == tSpelementUnitZUOutTypeUnit.Точка)
				{
