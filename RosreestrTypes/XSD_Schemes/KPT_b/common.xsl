...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:kpt="urn://x-artefacts-rosreestr-ru/outgoing/kpt/10.0.1" xmlns:adr="urn://x-artefacts-rosreestr-ru/commons/complex-types/address-output/4.0.1" xmlns:cert="urn://x-artefacts-rosreestr-ru/commons/complex-types/certification-doc/1.0" xmlns:doc="urn://x-artefacts-rosreestr-ru/commons/complex-types/document-output/4.0.1" xmlns:param="urn://x-artefacts-rosreestr-ru/commons/complex-types/parameters-oks/2.0.1" xmlns:spa="urn://x-artefacts-rosreestr-ru/commons/complex-types/entity-spatial/5.0.1" xmlns:tns="urn://x-artefacts-smev-gov-ru/supplementary/commons/1.0.1">
	<!--Версия: 31-->
	<!--22.05.2014 Убрано обрезание кадастровых номеров шаблоном ParcelCadNum-->
	<!--Версия: 32-->
	<!--27.11.2014 Новая версия схемы. СМЭВ-->
	<!--Версия: 33-->
	<!--02.12.2014 Добавлена нумерация листов-->
	<!--Версия: 34-->
	<!--03.12.2014 Новая версия схемы. Изменена структура объектов недвижимости в квартале-->
	<!--Версия: 35-->
	<!--03.12.2014 Новая подсхема документов. Новый НС. Элемент Number в Документах стал необязательным-->
	<!--Версия: 36-->
	<!--12.12.2014 Исправлено отображение графики на КПТ.2.3 и КПТ.2.4-->
	<!--Версия: 37-->
	<!--15.12.2014 На КПТ.4 убрано отображение поля Description для границ. Добавлено отображение КК даже если нет документов-->
	<!--Версия: 38-->
	<!--12.03.2015 Исправлено дублирование адреса-->
	<!--Версия: 39-->
	<!--24.04.2016 Новая версия схемы-->
	<!--24.05.2016 Убраны скобки при выводе КН УЧЛС. На КПТ 1.2 для УЧЛС проставлены прочерки. Убрано замыкание координат-->
	<!--Версия: 40-->
	<!--25.05.2016 Полностью переделана графика-->
	<!--26.05.2016 Переделан вывод РИ-->
	<!--Версия: 41-->
	<!--30.12.2016 Специальная версия по плану Б-->
	<!--05.01.2017 Убран раздел 6. Переделаны разделы 7 и 1-->
	<xsl:strip-space elements="*"/>
	<xsl:output method="html" indent="yes" omit-xml-declaration="yes" media-type="text/javascript" encoding="UTF-8"/>
	<!--Переменные-->
	<xsl:variable name="certificationDoc" select="kpt:KPT/kpt:CertificationDoc"/>
	...
	<xsl:variable name="objectsAll" select="($blocks/kpt:ObjectsRealty|$blocks/kpt:Parcels)/descendant::kpt:Parcel|descendant::kpt:Building|descendant::kpt:Construction|descendant::kpt:Uncompleted"/>
	<xsl:variable name="AreaTotal" select="$blocks/kpt:Area/kpt:Total"/>
	<xsl:variable name="spatials10" select="($blocks/kpt:Bounds/descendant::spa:SpatialElement)/spa:SpelementUnit"/>
	<xsl:variable name="spatials" select="($blocks/kpt:SpatialData/kpt:EntitySpatial/spa:SpatialElement|$blocks/kpt:Zones/descendant::spa:SpatialElement)/spa:SpelementUnit"/>
	<xsl:variable name="spatialsParcel" select="$parcels/descendant::spa:SpelementUnit"/>
	<xsl:variable name="spatialsObjects" select="$objects/descendant::spa:SpelementUnit"/>
	<xsl:variable name="decisions" select="$blocks/kpt:SpatialData|$blocks/descendant::kpt:Zone"/>
	<xsl:variable name="oms" select="$blocks/kpt:OMSPoints"/>
	...
	<xsl:variable name="max_page_records" select="5"/>
	<!--Количество участков-->
	<xsl:variable name="parcel_records" select="count($parcels/kpt:Parcel)"/>
	<!--Количество объектов недвижимости-->
	<xsl:variable name="object_records" select="count($objects/kpt:ObjectRealty)+count($parcels/kpt:Parcel)"/>
	<!--Количество решений-->
	<xsl:variable name="decision_records" select="count($decisions)"/>
	<!--Количество пунктов ОМС-->
	<xsl:variable name="oms_records" select="count($oms/kpt:OMSPoint)"/>
	<!--Количество границ объектов землеустройства, единиц кадастрового деления-->
	<xsl:variable name="spatial_records10" select="count($spatials10)"/>
	<xsl:variable name="spatial_records" select="count($spatials)"/>
	<!--Количество границ земельных участков-->
	<xsl:variable name="spatialParcels_records" select="count($spatialsParcel)"/>
	<!--Количество границ объектов недвижимости-->
	<xsl:variable name="spatialObjects_records" select="count($spatialsObjects)"/>
	<!--Количество листов КПТ1-->
	<xsl:variable name="kpt1listcount">
		<xsl:choose>
			<xsl:when test="$object_records &gt; $max_page_records">
				<xsl:value-of select="ceiling($object_records div $max_page_records)"/>
			</xsl:when>
			<xsl:when test="not($object_records)">
	...
	<!--Количество листов КПТ2-->
	<xsl:variable name="kpt2listcount">
		<xsl:if test="descendant::kpt:SpatialData/kpt:EntitySpatial|descendant::kpt:Parcel/descendant::kpt:EntitySpatial|descendant::kpt:ObjectsRealty/descendant::kpt:EntitySpatial">
			<xsl:value-of select="1"/>
		</xsl:if>
		<xsl:if test="not(descendant::kpt:SpatialData/kpt:EntitySpatial|descendant::kpt:Parcel/descendant::kpt:EntitySpatial|descendant::kpt:ObjectsRealty/descendant::kpt:EntitySpatial)">
			<xsl:value-of select="0"/>
		</xsl:if>
	</xsl:variable>
	<!--Количество листов КПТ3-->
	<xsl:variable name="kpt3listcount">
		<xsl:if test="descendant::kpt:Bound/descendant::kpt:EntitySpatial">
			<xsl:value-of select="1"/>
		</xsl:if>
		<xsl:if test="not(descendant::kpt:Bound/descendant::kpt:EntitySpatial)">
			<xsl:value-of select="0"/>
		</xsl:if>
	</xsl:variable>
	<!--Количество листов КПТ4-->
	<xsl:variable name="kpt4listcount">
		<xsl:if test="descendant::kpt:Zone[kpt:SpecialZone]/descendant::kpt:EntitySpatial">
			<xsl:value-of select="1"/>
		</xsl:if>
		<xsl:if test="not(descendant::kpt:Zone[kpt:SpecialZone]/descendant::kpt:EntitySpatial)">
			<xsl:value-of select="0"/>
		</xsl:if>
	</xsl:variable>
	<!--Количество листов КПТ5-->
	<xsl:variable name="kpt5listcount">
		<xsl:if test="descendant::kpt:Zone[kpt:TerritorialZone]/descendant::kpt:EntitySpatial">
			<xsl:value-of select="1"/>
		</xsl:if>
		<xsl:if test="not(descendant::kpt:Zone[kpt:TerritorialZone]/descendant::kpt:EntitySpatial)">
			<xsl:value-of select="0"/>
		</xsl:if>
	</xsl:variable>
	<!--Количество листов КПТ6-->
	<xsl:variable name="kpt6listcount">
		<xsl:if test="descendant::kpt:SpatialData/kpt:EntitySpatial">
			<xsl:value-of select="0"/>
		</xsl:if>
		<xsl:if test="not(descendant::kpt:SpatialData/kpt:EntitySpatial)">
			<xsl:value-of select="0"/>
		</xsl:if>
	</xsl:variable>
	<!--Количество листов КПТ7-->
	<xsl:variable name="kpt7listcount">
		<xsl:choose>
			<xsl:when test="$decision_records &gt; ($max_page_records+1)">
				<xsl:value-of select="ceiling($decision_records div ($max_page_records+1))"/>
			</xsl:when>
			<xsl:when test="not($decision_records)">
				<xsl:value-of select="0"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="1"/>
	...
	<!--Количество листов КПТ8-->
	<xsl:variable name="kpt8listcount">
		<xsl:choose>
			<xsl:when test="$spatialParcels_records &gt; ($max_page_records*4)">
				<xsl:value-of select="ceiling($spatialParcels_records div ($max_page_records*4))"/>
			</xsl:when>
			<xsl:when test="not($spatialParcels_records)">
				<xsl:value-of select="0"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="1"/>
	...
	<!--Количество листов КПТ9-->
	<xsl:variable name="kpt9listcount">
		<xsl:choose>
			<xsl:when test="$spatialObjects_records &gt; ($max_page_records*4)">
				<xsl:value-of select="ceiling($spatialObjects_records div ($max_page_records*4))"/>
			</xsl:when>
			<xsl:when test="not($spatialObjects_records)">
				<xsl:value-of select="0"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="1"/>
	...
	<!--Количество листов КПТ10-->
	<xsl:variable name="kpt10listcount">
		<xsl:choose>
			<xsl:when test="$spatial_records10 &gt; ($max_page_records*2)">
				<xsl:value-of select="ceiling($spatial_records10 div ($max_page_records*2))"/>
			</xsl:when>
			<xsl:when test="not($spatial_records10)">
				<xsl:value-of select="0"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="1"/>
	...
	<!--Количество листов КПТ11-->
	<xsl:variable name="kpt11listcount">
		<xsl:choose>
			<xsl:when test="$spatial_records &gt; ($max_page_records*2)">
				<xsl:value-of select="ceiling($spatial_records div ($max_page_records*2))"/>
			</xsl:when>
			<xsl:when test="not($spatial_records)">
				<xsl:value-of select="0"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="1"/>
	...
	<!--Количество листов КПТ12-->
	<xsl:variable name="kpt12listcount">
		<xsl:choose>
			<xsl:when test="$oms_records &gt; ($max_page_records*2)">
				<xsl:value-of select="ceiling($oms_records div ($max_page_records*2))"/>
			</xsl:when>
			<xsl:when test="not($oms_records)">
				<xsl:value-of select="0"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="1"/>
	...
	<!-- Всего листов -->
	<xsl:variable name="kptAllcount">
		<xsl:value-of select="$kpt1listcount+$kpt2listcount+$kpt3listcount+$kpt4listcount+$kpt5listcount+$kpt6listcount+$kpt7listcount+$kpt8listcount+$kpt9listcount+$kpt10listcount+$kpt11listcount+$kpt12listcount"/>
	</xsl:variable>
	<!--Наличие раздела КПТ1-->
	<xsl:variable name="kpt1article">
		<xsl:choose>
			<xsl:when test="$object_records!=0">
				<xsl:value-of select="1"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="0"/>
	...
	<!--Наличие раздела КПТ2-->
	<xsl:variable name="kpt2article">
		<xsl:if test="descendant::kpt:SpatialData/kpt:EntitySpatial|descendant::kpt:Parcel/descendant::kpt:EntitySpatial|descendant::kpt:ObjectsRealty/descendant::kpt:EntitySpatial">
			<xsl:value-of select="1"/>
		</xsl:if>
		<xsl:if test="not(descendant::kpt:SpatialData/kpt:EntitySpatial|descendant::kpt:Parcel/descendant::kpt:EntitySpatial|descendant::kpt:ObjectsRealty/descendant::kpt:EntitySpatial)">
	...
	<!--Наличие раздела КПТ3-->
	<xsl:variable name="kpt3article">
		<xsl:if test="descendant::kpt:Bound/descendant::kpt:EntitySpatial">
			<xsl:value-of select="1"/>
		</xsl:if>
		<xsl:if test="not(descendant::kpt:Bound/descendant::kpt:EntitySpatial)">
	...
	<!--Наличие раздела КПТ4-->
	<xsl:variable name="kpt4article">
		<xsl:if test="descendant::kpt:Zone[kpt:SpecialZone]/descendant::kpt:EntitySpatial">
			<xsl:value-of select="1"/>
		</xsl:if>
		<xsl:if test="not(descendant::kpt:Zone[kpt:SpecialZone]/descendant::kpt:EntitySpatial)">
	...
	<!--Наличие раздела КПТ5-->
	<xsl:variable name="kpt5article">
		<xsl:if test="descendant::kpt:Zone[kpt:TerritorialZone]/descendant::kpt:EntitySpatial">
			<xsl:value-of select="1"/>
		</xsl:if>
		<xsl:if test="not(descendant::kpt:Zone[kpt:TerritorialZone]/descendant::kpt:EntitySpatial)">
	...
	<!--Наличие раздела КПТ6-->
	<xsl:variable name="kpt6article">
		<xsl:if test="descendant::kpt:SpatialData/kpt:EntitySpatial">
			<xsl:value-of select="0"/>
		</xsl:if>
		<xsl:if test="not(descendant::kpt:SpatialData/kpt:EntitySpatial)">
	...
	<!--Наличие раздела КПТ7-->
	<xsl:variable name="kpt7article">
		<xsl:choose>
			<xsl:when test="$decision_records!=0">
				<xsl:value-of select="1"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="0"/>
	...
	<!--Наличие раздела КПТ8-->
	<xsl:variable name="kpt8article">
		<xsl:choose>
			<xsl:when test="$spatialParcels_records!=0">
				<xsl:value-of select="1"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="0"/>
	...
	<!--Наличие раздела КПТ9-->
	<xsl:variable name="kpt9article">
		<xsl:choose>
			<xsl:when test="$spatialObjects_records!=0">
				<xsl:value-of select="1"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="0"/>
	...
	<!--Наличие раздела КПТ10-->
	<xsl:variable name="kpt10article">
		<xsl:choose>
			<xsl:when test="$spatial_records10!=0">
				<xsl:value-of select="1"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="0"/>
	...
	<!--Наличие раздела КПТ11-->
	<xsl:variable name="kpt11article">
		<xsl:choose>
			<xsl:when test="$spatial_records!=0">
				<xsl:value-of select="1"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="0"/>
	...
	<!--Наличие раздела КПТ12-->
	<xsl:variable name="kpt12article">
		<xsl:choose>
			<xsl:when test="$oms_records!=0">
				<xsl:value-of select="1"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="0"/>
	...
	<!-- Всего разделов -->
	<xsl:variable name="kptAllcountArticles">
		<xsl:value-of select="$kpt1article+$kpt2article+$kpt3article+$kpt4article+$kpt5article+$kpt6article+$kpt7article+$kpt8article+$kpt9article+$kpt10article+$kpt11article+$kpt12article"/>
	</xsl:variable>
...
var Coords5 = new Array();

// Основная функция
function load(id, Coords, minX, minY, maxX, maxY) {
	var canvas;
	var context;
    ...
    setInterval(drawAll, 100, canvas, context, Coords, minX, maxX, minY, maxY);
    ...
    $(canvas).mousedown(function mouseMove(event)
    {
		...
		$(document).bind('mousemove.cropper', function (event) {
			...
			drawAll(canvas, context, Coords, minX, maxX, minY, maxY);
...
function drawAll(canvas, context, Coords, minX, maxX, minY, maxY) {
        ...
        for (var z = 0; z < Coords[i].length; z++) {
            ...
            if (Coords[i][z].length > 4) {
                ...
                for (var j = 1; j < Coords[i][z].length / 2; j++) {
					...
					if (Coords[i][0][0].charAt(0) == 'X')
						context.dashedLine(y2p, x2p, y2, x2, [2, 5]);
					else if (Coords[i][0][0].charAt(0) == 'X')
...
if (CP && CP.lineTo){
	CP.dashedLine = function(x,y,x2,y2,dashArray){
		...
		while (distRemaining>=0.1){
			...
			draw = !draw;
						...
						]]><xsl:apply-templates select="descendant::kpt:SpatialData/kpt:EntitySpatial|descendant::kpt:Parcel/descendant::kpt:EntitySpatial|descendant::kpt:ObjectsRealty/descendant::kpt:EntitySpatial" mode="kpt21">
							<xsl:with-param name="count" select="count(descendant::kpt:SpatialData/kpt:EntitySpatial|descendant::kpt:Parcel/descendant::kpt:EntitySpatial|descendant::kpt:ObjectsRealty/descendant::kpt:EntitySpatial)"/>
						</xsl:apply-templates>
						<xsl:apply-templates select="descendant::kpt:Bound/descendant::kpt:EntitySpatial" mode="kpt22">
							<xsl:with-param name="count" select="count(descendant::kpt:Bound/descendant::kpt:EntitySpatial)"/>
						</xsl:apply-templates>
						<xsl:apply-templates select="descendant::kpt:Zone[kpt:SpecialZone]/descendant::kpt:EntitySpatial" mode="kpt23">
							<xsl:with-param name="count" select="count(descendant::kpt:Zone[kpt:SpecialZone]/descendant::kpt:EntitySpatial)"/>
						</xsl:apply-templates>
						<xsl:apply-templates select="descendant::kpt:Zone[kpt:TerritorialZone]/descendant::kpt:EntitySpatial" mode="kpt24">
							<xsl:with-param name="count" select="count(descendant::kpt:Zone[kpt:TerritorialZone]/descendant::kpt:EntitySpatial)"/>
						</xsl:apply-templates>
						<xsl:apply-templates select="descendant::kpt:SpatialData/kpt:EntitySpatial" mode="kpt25">
							<xsl:with-param name="count" select="count(descendant::kpt:SpatialData/kpt:EntitySpatial)"/>
						</xsl:apply-templates>
					</script>
					<script type="text/javascript">
						<xsl:variable name="minX1">
							<xsl:for-each select="descendant::kpt:SpatialData/descendant::spa:Ordinate/@X|descendant::kpt:Parcel/descendant::spa:Ordinate/@X|descendant::kpt:ObjectsRealty/descendant::spa:Ordinate/@X">
						...
						<xsl:variable name="minY1">
							<xsl:for-each select="descendant::kpt:SpatialData/descendant::spa:Ordinate/@Y|descendant::kpt:Parcel/descendant::spa:Ordinate/@Y|descendant::kpt:ObjectsRealty/descendant::spa:Ordinate/@Y">
								<xsl:sort data-type="number" order="ascending"/>
								<xsl:if test="position()=1">
									<xsl:value-of select="."/>
								</xsl:if>
						...
						<xsl:variable name="minX2">
							<xsl:for-each select="descendant::kpt:Bound/descendant::spa:Ordinate/@X">
								<xsl:sort data-type="number" order="ascending"/>
								<xsl:if test="position()=1">
									<xsl:value-of select="."/>
								</xsl:if>
						...
						<xsl:variable name="minY2">
							<xsl:for-each select="descendant::kpt:Bound/descendant::spa:Ordinate/@Y">
								<xsl:sort data-type="number" order="ascending"/>
								<xsl:if test="position()=1">
									<xsl:value-of select="."/>
								</xsl:if>
						...
						<xsl:variable name="minX3">
							<xsl:for-each select="descendant::kpt:Zone[kpt:SpecialZone]/descendant::spa:Ordinate/@X">
								<xsl:sort data-type="number" order="ascending"/>
								<xsl:if test="position()=1">
									<xsl:value-of select="."/>
								</xsl:if>
						...
						<xsl:variable name="minY3">
							<xsl:for-each select="descendant::kpt:Zone[kpt:SpecialZone]/descendant::spa:Ordinate/@Y">
								<xsl:sort data-type="number" order="ascending"/>
								<xsl:if test="position()=1">
									<xsl:value-of select="."/>
								</xsl:if>
						...
						<xsl:variable name="minX4">
							<xsl:for-each select="descendant::kpt:Zone[kpt:TerritorialZone]/descendant::spa:Ordinate/@X">
								<xsl:sort data-type="number" order="ascending"/>
								<xsl:if test="position()=1">
									<xsl:value-of select="."/>
								</xsl:if>
						...
						<xsl:variable name="minY4">
							<xsl:for-each select="descendant::kpt:Zone[kpt:TerritorialZone]/descendant::spa:Ordinate/@Y">
								<xsl:sort data-type="number" order="ascending"/>
								<xsl:if test="position()=1">
									<xsl:value-of select="."/>
								</xsl:if>
						...
						<!--xsl:variable name="minX5">
							<xsl:for-each select="descendant::kpt:SpatialData/kpt:EntitySpatial/descendant::spa:Ordinate/@X">
								<xsl:sort data-type="number" order="ascending"/>
								<xsl:if test="position()=1">
									<xsl:value-of select="."/>
								</xsl:if>
						...
						<xsl:variable name="minY5">
							<xsl:for-each select="descendant::kpt:SpatialData/kpt:EntitySpatial/descendant::spa:Ordinate/@Y">
								<xsl:sort data-type="number" order="ascending"/>
								<xsl:if test="position()=1">
									<xsl:value-of select="."/>
								</xsl:if>
						...
						<xsl:if test="descendant::kpt:SpatialData/kpt:EntitySpatial|descendant::kpt:Parcel/descendant::kpt:EntitySpatial|descendant::kpt:ObjectsRealty/descendant::kpt:EntitySpatial">
							<xsl:text>var minX1 = </xsl:text>
							<xsl:value-of select="$minX1"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var minY1 = </xsl:text>
							<xsl:value-of select="$minY1"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var maxX1 = </xsl:text>
							<xsl:value-of select="$maxX1"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var maxY1 = </xsl:text>
							<xsl:value-of select="$maxY1"/>
							<xsl:text>;</xsl:text>
						</xsl:if>
						<xsl:if test="not(descendant::kpt:SpatialData/kpt:EntitySpatial|descendant::kpt:Parcel/descendant::kpt:EntitySpatial|descendant::kpt:ObjectsRealty/descendant::kpt:EntitySpatial)">
							<xsl:text>var minX1 = 0;</xsl:text>
							<xsl:text>var minY1 = 0;</xsl:text>
							<xsl:text>var maxX1 = 0;</xsl:text>
							<xsl:text>var maxY1 = 0;</xsl:text>
						</xsl:if>
						<xsl:if test="descendant::kpt:Bound/descendant::kpt:EntitySpatial">
							<xsl:text>var minX2 = </xsl:text>
							<xsl:value-of select="$minX2"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var minY2 = </xsl:text>
							<xsl:value-of select="$minY2"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var maxX2 = </xsl:text>
							<xsl:value-of select="$maxX2"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var maxY2 = </xsl:text>
							<xsl:value-of select="$maxY2"/>
							<xsl:text>;</xsl:text>
						</xsl:if>
						<xsl:if test="not(descendant::kpt:Bound/descendant::kpt:EntitySpatial)">
							<xsl:text>var minX2 = 0;</xsl:text>
							<xsl:text>var minY2 = 0;</xsl:text>
							<xsl:text>var maxX2 = 0;</xsl:text>
							<xsl:text>var maxY2 = 0;</xsl:text>
						</xsl:if>
						<xsl:if test="descendant::kpt:Zone[kpt:SpecialZone]/descendant::kpt:EntitySpatial">
							<xsl:text>var minX3 = </xsl:text>
							<xsl:value-of select="$minX3"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var minY3 = </xsl:text>
							<xsl:value-of select="$minY3"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var maxX3 = </xsl:text>
							<xsl:value-of select="$maxX3"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var maxY3 = </xsl:text>
							<xsl:value-of select="$maxY3"/>
							<xsl:text>;</xsl:text>
						</xsl:if>
						<xsl:if test="not(descendant::kpt:Zone[kpt:SpecialZone]/descendant::kpt:EntitySpatial)">
							<xsl:text>var minX3 = 0;</xsl:text>
							<xsl:text>var minY3 = 0;</xsl:text>
							<xsl:text>var maxX3 = 0;</xsl:text>
							<xsl:text>var maxY3 = 0;</xsl:text>
						</xsl:if>
						<xsl:if test="descendant::kpt:Zone[kpt:TerritorialZone]/descendant::kpt:EntitySpatial">
							<xsl:text>var minX4 = </xsl:text>
							<xsl:value-of select="$minX4"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var minY4 = </xsl:text>
							<xsl:value-of select="$minY4"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var maxX4 = </xsl:text>
							<xsl:value-of select="$maxX4"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var maxY4 = </xsl:text>
							<xsl:value-of select="$maxY4"/>
							<xsl:text>;</xsl:text>
						</xsl:if>
						<xsl:if test="not(descendant::kpt:Zone[kpt:TerritorialZone]/descendant::kpt:EntitySpatial)">
							<xsl:text>var minX4 = 0;</xsl:text>
							<xsl:text>var minY4 = 0;</xsl:text>
							<xsl:text>var maxX4 = 0;</xsl:text>
							<xsl:text>var maxY4 = 0;</xsl:text>
						</xsl:if>
						<!--xsl:if test="descendant::kpt:SpatialData/kpt:EntitySpatial">
							<xsl:text>var minX5 = </xsl:text>
							<xsl:value-of select="$minX5"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var minY5 = </xsl:text>
							<xsl:value-of select="$minY5"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var maxX5 = </xsl:text>
							<xsl:value-of select="$maxX5"/>
							<xsl:text>;</xsl:text>
							<xsl:text>var maxY5 = </xsl:text>
							<xsl:value-of select="$maxY5"/>
							<xsl:text>;</xsl:text>
						</xsl:if>
						<xsl:if test="not(descendant::kpt:SpatialData/kpt:EntitySpatial)">
							<xsl:text>var minX5 = 0;</xsl:text>
							<xsl:text>var minY5 = 0;</xsl:text>
							<xsl:text>var maxX5 = 0;</xsl:text>
							<xsl:text>var maxY5 = 0;</xsl:text>
						</xsl:if-->
					</script>
				</xsl:if>
			</head>
			<xsl:element name="body">
				<xsl:attribute name="onload"><xsl:if test="descendant::kpt:SpatialData/kpt:EntitySpatial|descendant::kpt:Parcel/descendant::kpt:EntitySpatial|descendant::kpt:ObjectsRealty/descendant::kpt:EntitySpatial"><xsl:text>load('canvas1', Coords1, minX1, minY1, maxX1, maxY1);</xsl:text></xsl:if><xsl:if test="descendant::kpt:Bound/descendant::kpt:EntitySpatial"><xsl:text>load('canvas2', Coords2, minX2, minY2, maxX2, maxY2);</xsl:text></xsl:if><xsl:if test="descendant::kpt:Zone[kpt:SpecialZone]/descendant::kpt:EntitySpatial"><xsl:text>load('canvas3', Coords3, minX3, minY3, maxX3, maxY3);</xsl:text></xsl:if><xsl:if test="descendant::kpt:Zone[kpt:TerritorialZone]/descendant::kpt:EntitySpatial"><xsl:text>load('canvas4', Coords4, minX4, minY4, maxX4, maxY4);</xsl:text></xsl:if></xsl:attribute>
				<xsl:apply-templates/>
			</xsl:element>
		</html>
	</xsl:template>
	...
	<!--Заголовок страницы с номером листа в параметре-->
		...
		<table class="tbl_section_title">
			<tr>
				...
				<td style="text-align: right; white-space:nowrap;">
					<xsl:value-of select="concat('Раздел ', $pageNumber)"/>
				</td>
			</tr>
			<tr>
				<td style="text-align: center; width: 100%">КАДАСТРОВЫЙ ПЛАН ТЕРРИТОРИИ</td>
			</tr>
		</table>
	</xsl:template>
	<!--Выводит дату в заголовке-->
	<xsl:template name="section_date">
		<table class="tbl_section_date">
			<tr>
				<td style="border: 0; padding-bottom: 0">&laquo;</td>
				<td style="border: 0; border-bottom: 1px solid #000; padding-bottom: 0" class="understroke">
					<xsl:value-of select="$day"/>
				</td>
				<td style="border: 0; padding-bottom: 0">&raquo;</td>
				<td style="border: 0; border-bottom: 1px solid #000; padding-bottom: 0" class="understroke">
					<xsl:variable name="var" select="document('dict/months.xml')"/>
					<xsl:value-of select="$var/row_list/row[CODE=$month]/NAME"/>
				</td>
				<td style="border: 0; padding-bottom: 0" class="norpad">
					<xsl:value-of select="$year"/>&nbsp;г.&nbsp;
				</td>
				<td style="border: 0; padding-bottom: 0" class="nolpad">&number;</td>
				<td class="understroke" style="white-space:nowrap; border: 0; border-bottom: 1px solid #000; padding-bottom: 0">
					<xsl:value-of select="$certificationDoc/cert:Number"/>
				</td>
			</tr>
		</table>
	</xsl:template>
	<!--Выводит количество листов для раздела и всего-->
	<xsl:template name="section_page">
		<xsl:param name="listNumber"/>
		<xsl:param name="pageNumber"/>
		<xsl:param name="pageAllCount"/>
		<table class="tbl_section_content">
			<tr>
				<td style="border-top: 0; border-bottom: 0" width="25%">
					<b>
						<xsl:value-of select="concat('Лист № ', $listNumber)"/>
						<span style="float: right">раздела <xsl:value-of select="$pageNumber"/>
						</span>
					</b>
				</td>
				<td style="border-top: 0; border-bottom: 0; border-right: 0" width="25%">
					<b>
						<xsl:text>Всего листов раздела </xsl:text>
						<xsl:value-of select="$pageNumber"/>: <xsl:value-of select="$pageAllCount"/>
					</b>
				</td>
				<td style="border-top: 0; border-bottom: 0; border-right: 0" width="25%">
					<b>
						<xsl:text>Всего разделов: </xsl:text>
						<xsl:value-of select="$kptAllcountArticles"/>
					</b>
				</td>
				<td style="border-top: 0; border-bottom: 0; border-right: 0" width="25%">
					<b>
						<xsl:text>Всего листов: </xsl:text>
						<xsl:value-of select="$kptAllcount"/>
					</b>
				</td>
			</tr>
			<tr>
			...
			<xsl:if test="($pageNumber=1) and ($listNumber=1)">
				<tr>
					<td colspan="2" style="border-top: 0; border-bottom: 0; border-left: 0">
						<b>
					...
					<td colspan="2">
						<b>
							<nobr>Площадь кадастрового квартала: </nobr>
							<nobr>
								<xsl:value-of select="$AreaTotal"/>
								<xsl:text>&nbsp;Га</xsl:text>
							</nobr>
						</b>
					</td>
				</tr>
			</xsl:if>
			<xsl:if test="(($pageNumber=1) and ($listNumber!=1)) or ($pageNumber!=1)">
				<tr>
					<td colspan="4" style="border-top: 0; border-bottom: 0; border-left: 0">
						<b>
							<nobr>Номер кадастрового квартала: </nobr>
							<nobr>
								<xsl:value-of select="$CadastralNumber"/>
	...
	<!--Шаблон нижней подписи-->
	<xsl:template name="section_sign">
		<table class="tbl_section_sign">
			<tr>
				...
				<td class="understroke">
					<b>
						<xsl:if test="$certificationDoc/cert:Official">
							<xsl:text>/&nbsp;</xsl:text>
	...
	<!--Форматирование даты-->
	<xsl:template match="cert:Date|kpt:Date|doc:Date">
		<xsl:value-of select="substring(.,9,2)"/>.<xsl:value-of select="substring(.,6,2)"/>.<xsl:value-of select="substring(.,1,4)"/>
	</xsl:template>
	<!--Шаблон категории земель-->
	<xsl:template match="kpt:Category" mode="short">
		<xsl:if test="text()='003001000000'">1</xsl:if>
		<xsl:if test="text()='003002000000'">2</xsl:if>
		<xsl:if test="text()='003003000000'">3</xsl:if>
		<xsl:if test="text()='003004000000'">4</xsl:if>
		<xsl:if test="text()='003005000000'">5</xsl:if>
		<xsl:if test="text()='003006000000'">6</xsl:if>
		<xsl:if test="text()='003007000000'">7</xsl:if>
		<xsl:if test="text()='003008000000'">8</xsl:if>
	</xsl:template>
	<xsl:template match="kpt:Category" mode="describe">
		<xsl:variable name="categories" select="document('schema/KPT_v10/SchemaCommon/dCategories_v01.xsd')"/>
		<xsl:variable name="kod" select="text()"/>
		<xsl:value-of select="$categories//xs:enumeration[@value=$kod]/xs:annotation/xs:documentation"/>
	</xsl:template>
	<xsl:template match="kpt:Category" mode="distinct">
		<xsl:if test="not(preceding::kpt:Category=.)">
			<xsl:value-of select="substring(string(.),6,1)"/>&nbsp;-&nbsp;
			<xsl:apply-templates select=".." mode="describe"/>
			<br/>
		</xsl:if>
	</xsl:template>
	<!--Шаблон разрешенного использования-->
	<xsl:template match="kpt:Utilization">
		<xsl:if test="string(@LandUse)">
			<xsl:variable name="resolvedLand" select="document('schema/KPT_v10/SchemaCommon/dAllowedUse_v02.xsd')"/>
			<xsl:variable name="kodLand" select="@LandUse"/>
			<xsl:value-of select="$resolvedLand//xs:enumeration[@value=$kodLand]/xs:annotation/xs:documentation"/>
		</xsl:if>
		<xsl:if test="string(@ByDoc) and not(string(@LandUse))">
			<xsl:value-of select="@ByDoc"/>
		</xsl:if>
		<xsl:if test="not(string(@ByDoc)) and not(string(@LandUse))">
			<xsl:variable name="resolved" select="document('schema/KPT_v10/SchemaCommon/dUtilizations_v01.xsd')"/>
			<xsl:variable name="kod" select="@Utilization"/>
			<xsl:value-of select="$resolved//xs:enumeration[@value=$kod]/xs:annotation/xs:documentation"/>
			<xsl:if test="not(string($resolved//xs:enumeration[@value=$kod]/xs:annotation/xs:documentation))">
				<xsl:call-template name="procherk"/>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	<!--Шаблон местоположения-->
	<xsl:template match="kpt:Location">
		<xsl:if test="string(kpt:inBounds) and kpt:inBounds!='2'">
			<xsl:text>Местоположение установлено относительно ориентира, расположенного </xsl:text>
			<xsl:if test="kpt:inBounds='1'">
				<xsl:text>в границах участка. </xsl:text>
			</xsl:if>
			<xsl:if test="kpt:inBounds='0'">
				<xsl:text>за пределами участка. </xsl:text>
			</xsl:if>
			<xsl:if test="kpt:Elaboration/kpt:ReferenceMark">
				<xsl:text>Ориентир </xsl:text>
				<xsl:value-of select="kpt:Elaboration/kpt:ReferenceMark"/>
				<xsl:text>. </xsl:text>
			</xsl:if>
			<xsl:if test="kpt:Elaboration/kpt:Direction">
				<xsl:text>Участок находится примерно в </xsl:text>
				<xsl:value-of select="kpt:Elaboration/kpt:Distance"/>
				<xsl:text> от ориентира по направлению на </xsl:text>
				<xsl:value-of select="kpt:Elaboration/kpt:Direction"/>
				<xsl:text>. </xsl:text>
			</xsl:if>
			<br/>
			<xsl:text>Почтовый адрес ориентира: </xsl:text>
			<xsl:apply-templates select="kpt:Address"/>
		</xsl:if>
		<xsl:if test="not(string(kpt:inBounds))">
			<xsl:apply-templates select="kpt:Address"/>
		</xsl:if>
	</xsl:template>
	<!--Шаблон адреса-->
	<xsl:template match="kpt:Address">
		<xsl:if test="not(string(adr:Note))">
			...
			<xsl:if test="string(adr:Region)">
				<xsl:variable name="region" select="document('schema/KPT_v10/SchemaCommon/dRegionsRF_v01.xsd')"/>
	...
	<!--Шаблон для участков-->
	<xsl:template match="kpt:Parcels">
		<xsl:param name="start_number"/>
		<xsl:apply-templates select="kpt:Parcel">
			<xsl:with-param name="start_number" select="$start_number"/>
			<xsl:sort select="number(substring(@CadastralNumber, 14))" data-type="number"/>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template match="kpt:Parcel">
		<xsl:param name="start_number"/>
		<xsl:if test="position() &gt; $start_number and position() &lt; ($start_number + $max_page_records)+1">
			<tr>
				<td class="td_center">
				...
				<td class="td_center">
					<xsl:value-of select="@CadastralNumber"/>
					<xsl:if test="kpt:Contours">
						<br/>
						<xsl:text>(Граница участка многоконтурная. Количество контуров - </xsl:text>
						<xsl:value-of select="count(kpt:Contours/kpt:Contour)"/>
						<xsl:text>)</xsl:text>
					</xsl:if>
					<xsl:if test="kpt:ParentCadastralNumbers">
						<br/>
						<xsl:text>(входит в единое землепользование </xsl:text>
						<xsl:value-of select="kpt:ParentCadastralNumbers/kpt:CadastralNumber"/>
						<xsl:text>)</xsl:text>
					</xsl:if>
				</td>
				<!--Вид объекта недвижимости-->
				<td class="td_center">Земельный участок</td>
				<!--Адрес-->
				<td class="td_center">
					<xsl:apply-templates select="kpt:Location"/>
				</td>
				<!--Площадь или основная характеристика-->
				<td class="td_center">
					<xsl:if test="string(kpt:Area/kpt:Area)">
						<xsl:value-of select="kpt:Area/kpt:Area"/>
						<xsl:if test="string(kpt:Area/kpt:Inaccuracy)">
							<xsl:text>&nbsp;+/-</xsl:text>
							<xsl:value-of select="kpt:Area/kpt:Inaccuracy"/>
						</xsl:if>
					</xsl:if>
					<xsl:if test="not(string(kpt:Area/kpt:Area))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
				<!--Категория земель-->
				<td class="td_center">
					<xsl:apply-templates select="kpt:Category" mode="describe"/>
				</td>
				<!--Виды разрешенного использования-->
				<td class="td_center">
					<xsl:apply-templates select="kpt:Utilization"/>
				</td>
				...
				<td class="td_center">
					<xsl:if test="kpt:CadastralCost">
						<xsl:value-of select="kpt:CadastralCost/@Value"/>
					</xsl:if>
					<xsl:if test="not(kpt:CadastralCost)">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
			</tr>
	...
	<!--Шаблон для объектов недвижимости-->
	<xsl:template match="kpt:ObjectsRealty">
		<xsl:param name="start_number"/>
		<xsl:apply-templates select="kpt:ObjectRealty/kpt:Building|kpt:ObjectRealty/kpt:Construction|kpt:ObjectRealty/kpt:Uncompleted">
			<xsl:with-param name="start_number" select="$start_number"/>
			<xsl:sort select="number(substring(@CadastralNumber, 14))" data-type="number"/>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template match="kpt:Building|kpt:Construction|kpt:Uncompleted">
		<xsl:param name="start_number"/>
		<xsl:if test="position() &gt; $start_number and position() &lt; ($start_number + ($max_page_records+1))">
			<tr>
				<td class="td_center">
				...
				<!--Вид объекта недвижимости-->
				<td class="td_center">
					<xsl:variable name="realty" select="document('schema/KPT_v10/SchemaCommon/dRealty_v03.xsd')"/>
					<xsl:variable name="kodr" select="kpt:ObjectType"/>
					<xsl:call-template name="smallCase">
						<xsl:with-param name="text" select="$realty//xs:enumeration[@value=$kodr]/xs:annotation/xs:documentation"/>
					</xsl:call-template>
				</td>
				<!--Адрес-->
				<td class="td_center">
					<xsl:if test="not(kpt:ConditionalPartLinear)">
						<xsl:apply-templates select="kpt:Address"/>
					</xsl:if>
					<xsl:if test="kpt:ConditionalPartLinear">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
				<!--Площадь или основная характеристика-->
				<td class="td_center">
					<xsl:if test="not(kpt:ConditionalPartLinear)">
						<xsl:if test="string(kpt:Area)">
							<xsl:value-of select="kpt:Area"/>
						</xsl:if>
						<xsl:if test="kpt:KeyParameters">
							<xsl:variable name="params" select="document('schema/KPT_v10/SchemaCommon/dTypeParameter_v01.xsd')"/>
							<xsl:for-each select="kpt:KeyParameters/param:KeyParameter">
								<xsl:variable name="kodp" select="@Type"/>
								<xsl:call-template name="smallCase">
									<xsl:with-param name="text">
										<xsl:value-of select="concat($params//xs:enumeration[@value=$kodp]/xs:annotation/xs:documentation,': ',@Value)"/>
										<xsl:choose>
						...
						<xsl:if test="not(string(kpt:Area)) and not(kpt:KeyParameters)">
							<xsl:call-template name="procherk"/>
						</xsl:if>
					</xsl:if>
					<xsl:if test="kpt:ConditionalPartLinear">
				...
				<td class="td_center">
					<xsl:if test="not(kpt:ConditionalPartLinear)">
						<xsl:if test="kpt:AssignationBuilding">
							<xsl:variable name="assign" select="document('schema/KPT_v10/SchemaCommon/dAssBuilding_v01.xsd')"/>
							<xsl:variable name="koda" select="kpt:AssignationBuilding"/>
							<xsl:value-of select="$assign//xs:enumeration[@value=$koda]/xs:annotation/xs:documentation"/>
						</xsl:if>
						<xsl:if test="string(kpt:AssignationName)">
							<xsl:value-of select="kpt:AssignationName"/>
						</xsl:if>
						<xsl:if test="not(string(kpt:AssignationName)) and not(kpt:AssignationBuilding)">
							<xsl:call-template name="procherk"/>
						</xsl:if>
					</xsl:if>
					<xsl:if test="kpt:ConditionalPartLinear">
	...
	<!--Шаблон для пунктов ОМС-->
	<xsl:template match="kpt:OMSPoints">
		<xsl:param name="start_number"/>
		<xsl:apply-templates select="kpt:OMSPoint">
			<xsl:with-param name="start_number" select="$start_number"/>
			<xsl:sort select="number($oms/kpt:OMSPoint/kpt:PNmb)" data-type="number"/>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template match="kpt:OMSPoint">
		<xsl:param name="start_number"/>
		<xsl:if test="position() &gt; $start_number and position() &lt; ($start_number + ($max_page_records*2))+1">
			<tr>
				<!--Номер пункта опорной межевой сети на плане-->
				<td class="td_center">
					<xsl:value-of select="kpt:PNmb"/>
					<xsl:if test="not(string(kpt:PNmb))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
				<!--Название и (или) номер, тип пункта опорной межевой сети-->
				<td class="td_center">
					<xsl:value-of select="kpt:PName"/>
					<xsl:if test="not(string(kpt:PName))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
				<!--Класс опорной межевой сети-->
				<td class="td_center">
					<xsl:value-of select="kpt:PKlass"/>
					<xsl:if test="not(string(kpt:PKlass))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
				<!--Координата X-->
				<td class="td_center">
					<xsl:value-of select="kpt:OrdX"/>
					<xsl:if test="not(string(kpt:OrdX))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
				<!--Координата Y-->
				<td class="td_center">
					<xsl:value-of select="kpt:OrdY"/>
					<xsl:if test="not(string(kpt:OrdY))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
			</tr>
	...
	<!--Шаблон для решений о границах-->
	<xsl:template match="kpt:Documents">
		<xsl:param name="start_number"/>
		<xsl:apply-templates select="kpt:Document">
			<xsl:with-param name="parent" select=".."/>
			<xsl:with-param name="start_number" select="$start_number"/>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template match="kpt:Document">
		<xsl:param name="parent" select="../.."/>
		<xsl:param name="start_number"/>
		<xsl:if test="position() &gt; $start_number and position() &lt; ($start_number + ($max_page_records+1))+1">
			<tr>
				<td class="td_center">
					<xsl:value-of select="position()"/>
				</td>
				<!--Номер объекта землеустройства-->
				<td class="td_center">
					<xsl:value-of select="$parent/kpt:AccountNumber|$parent/parent::node()/@CadastralNumber"/>
					<xsl:if test="not(string($parent/kpt:AccountNumber|$parent/parent::node()/@CadastralNumber))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
				<!--Вид и наименование объекта землеустройства-->
				<td class="td_center">
					<xsl:call-template name="procherk"/>
					<!--xsl:choose>
						<xsl:when test="$parent/kpt:SubjectsBoundary">
							<xsl:value-of select="concat('граница между субъектами Российской Федерации ',$parent/kpt:SubjectsBoundary/kpt:NameNeighbours[1],' и ',$parent/kpt:SubjectsBoundary/kpt:NameNeighbours[2])"/>
						</xsl:when>
						<xsl:when test="$parent/kpt:MunicipalBoundary">
							<xsl:value-of select="concat('граница муниципального образования ',$parent/kpt:MunicipalBoundary/kpt:Name)"/>
						</xsl:when>
						<xsl:when test="$parent/kpt:InhabitedLocalityBoundary">
							<xsl:value-of select="concat('граница населенного пункта ', $parent/kpt:InhabitedLocalityBoundary/kpt:Name)"/>
						</xsl:when>
						<xsl:when test="$parent/kpt:TerritorialZone">
							<xsl:value-of select="concat('граница территориальной зоны ',$parent/kpt:Description)"/>
						</xsl:when>
						<xsl:when test="$parent/kpt:SpecialZone">
							<xsl:value-of select="concat('граница зоны с особыми условиями использования территории ',$parent/kpt:Description)"/>
						</xsl:when>
						<xsl:when test="$parent/self::kpt:SpatialData">
							<xsl:text>граница кадастрового квартала</xsl:text>
						</xsl:when>
					</xsl:choose-->
				</td>
				<!--Реквизиты решений-->
				<!--td class="td_center">
					<xsl:if test="string(doc:Name)">
						<xsl:value-of select="doc:Name"/>
					</xsl:if>
					<xsl:if test="not(string(doc:Name))">
						<xsl:variable name="ddoc" select="document('schema/KPT_v10/SchemaCommon/dAllDocumentsOut_v03.xsd')"/>
						<xsl:variable name="kod" select="doc:CodeDocument"/>
						<xsl:value-of select="$ddoc//xs:enumeration[@value=$kod]/xs:annotation/xs:documentation"/>
					</xsl:if>
					...
					<xsl:if test="string(doc:IssueOrgan)">
						<xsl:text>&nbsp;выдан&nbsp;</xsl:text>
						<xsl:value-of select="doc:IssueOrgan"/>
					</xsl:if>
					<xsl:if test="not(string(doc:Number)) and not(string(doc:Date)) and not(string(doc:IssueOrgan)) and not(string(doc:Name)) and not(string(doc:CodeDocument))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td-->
				<!--Сфера ограничений-->
				<td class="td_center">
					<xsl:if test="$parent/kpt:TerritorialZone">
						<xsl:variable name="permituse" select="document('schema/KPT_v10/SchemaCommon/dPermitUse_v01.xsd')"/>
						<xsl:variable name="utilization" select="document('schema/KPT_v10/SchemaCommon/dUtilizations_v01.xsd')"/>
						<xsl:variable name="landuse" select="document('schema/KPT_v10/SchemaCommon/dAllowedUse_v02.xsd')"/>
						<xsl:for-each select="$parent/kpt:TerritorialZone/kpt:PermittedUses/kpt:PermittedUse">
							<xsl:variable name="kod" select="kpt:TypePermittedUse"/>
							<xsl:value-of select="$permituse//xs:enumeration[@value=$kod]/xs:annotation/xs:documentation"/>
							<xsl:if test="kpt:LandUse">
								<xsl:if test="kpt:TypePermittedUse">
									<xsl:text>,&nbsp;</xsl:text>
								</xsl:if>
								<xsl:variable name="kodl" select="kpt:LandUse"/>
								<xsl:value-of select="$landuse//xs:enumeration[@value=$kodl]/xs:annotation/xs:documentation"/>
							</xsl:if>
							<xsl:if test="string(kpt:PermittedUse) and not(kpt:LandUse)">
								<xsl:if test="kpt:TypePermittedUse">
									<xsl:text>,&nbsp;</xsl:text>
								</xsl:if>
								<xsl:value-of select="kpt:PermittedUse"/>
							</xsl:if>
							<xsl:if test="string(kpt:Utilization) and not(string(kpt:PermittedUse)) and not(kpt:LandUse)">
								<xsl:if test="kpt:TypePermittedUse">
									<xsl:text>,&nbsp;</xsl:text>
								</xsl:if>
								<xsl:variable name="kodu" select="kpt:Utilization"/>
								<xsl:value-of select="$utilization//xs:enumeration[@value=$kodu]/xs:annotation/xs:documentation"/>
							</xsl:if>
							<xsl:if test="kpt:PermitedAncillary">,&nbsp;</xsl:if>
							<xsl:for-each select="kpt:PermitedAncillary">
								<xsl:variable name="koda" select="kpt:TypePermittedUse"/>
								<xsl:value-of select="$permituse//xs:enumeration[@value=$koda]/xs:annotation/xs:documentation"/>
								<xsl:if test="kpt:LandUse">
									<xsl:if test="kpt:TypePermittedUse">
										<xsl:text>,&nbsp;</xsl:text>
									</xsl:if>
					...
					<xsl:if test="$parent/kpt:SpecialZone">
						<xsl:value-of select="$parent/kpt:SpecialZone/kpt:ContentRestrictions"/>
					</xsl:if>
					<xsl:if test="not(string($parent/kpt:SpecialZone/kpt:ContentRestrictions)) and not(string($parent/kpt:TerritorialZone/kpt:PermittedUses/kpt:PermittedUse/kpt:PermittedUse)) and not(string($parent/kpt:TerritorialZone/kpt:PermittedUses/kpt:PermittedUse/kpt:LandUse))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
			</tr>
	...
	<xsl:template match="kpt:SpatialData|kpt:Zone">
		<xsl:param name="parent" select=".."/>
		<xsl:param name="start_number"/>
		<xsl:if test="position() &gt; $start_number and position() &lt; ($start_number + ($max_page_records+1))+1">
			<tr>
				...
				<!--Номер объекта землеустройства-->
				<td class="td_center">
					<xsl:value-of select="$parent/@CadastralNumber|kpt:AccountNumber"/>
					<xsl:if test="not(string($parent/@CadastralNumber)) and not(string(kpt:AccountNumber))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
				<!--Реквизиты решений-->
				...
				<!--Сфера ограничений-->
				<td class="td_center">
					<xsl:if test="self::kpt:SpatialData">граница кадастрового квартала</xsl:if>
					<xsl:if test="not(self::kpt:SpatialData)">
						<xsl:choose>
							<xsl:when test="kpt:TerritorialZone">
								<xsl:value-of select="concat('граница территориальной зоны ',$parent/kpt:Description)"/>
							</xsl:when>
							<xsl:when test="kpt:SpecialZone">
								<xsl:value-of select="concat('граница зоны с особыми условиями использования территории ',$parent/kpt:Description)"/>
							</xsl:when>
						</xsl:choose>
					</xsl:if>
	...
	<!--Шаблон для описания границ объектов-->
	<xsl:template match="spa:SpelementUnit" mode="kpt31">
		<xsl:param name="start_number"/>
		<xsl:if test="position() &gt; $start_number and position() &lt; ($start_number + ($max_page_records*4))+1">
			<tr>
				<td class="td_center">
					<xsl:if test="../../parent::kpt:Parcel">
						<xsl:value-of select="../../../@CadastralNumber"/>
					</xsl:if>
					<xsl:if test="../../parent::kpt:Contour">
						<xsl:value-of select="concat(../../../../../@CadastralNumber,'(',../../../@NumberRecord,')')"/>
					</xsl:if>
				</td>
				<td class="td_center">
					<xsl:if test="count(ancestor::kpt:EntitySpatial/spa:SpatialElement)>1">
				...
				<td class="td_center">
					<xsl:if test="string(spa:Ordinate/@GeopointOpred)">
						<xsl:variable name="categories" select="document('schema/KPT_v10/SchemaCommon/dGeopointOpred_v01.xsd')"/>
						<xsl:variable name="kod" select="spa:Ordinate/@GeopointOpred"/>
						<xsl:value-of select="$categories//xs:enumeration[@value=$kod]/xs:annotation/xs:documentation"/>
						<xsl:if test="spa:Ordinate/@DeltaGeopoint">; </xsl:if>
					</xsl:if>
					<xsl:value-of select="spa:Ordinate/@DeltaGeopoint"/>
					<xsl:if test="not(string(spa:Ordinate/@DeltaGeopoint)) and not(string(spa:Ordinate/@GeopointOpred))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
			</tr>
	...
	<xsl:template match="spa:SpelementUnit" mode="kpt32">
		<xsl:param name="start_number"/>
		<xsl:if test="position() &gt; $start_number and position() &lt; ($start_number + ($max_page_records*4))+1">
			<tr>
				<td class="td_center">
					<xsl:if test="ancestor::kpt:ConditionalPartLinear">
						<xsl:value-of select="substring-before(string(ancestor::kpt:ConditionalPartLinear/@CadastralNumber),'(')"/>
					</xsl:if>
					<xsl:if test="not(ancestor::kpt:ConditionalPartLinear)">
						<xsl:value-of select="../../../@CadastralNumber"/>
					</xsl:if>
				</td>
				<td class="td_center">
				...
				<td class="td_center">
					<xsl:value-of select="spa:R"/>
					<xsl:if test="not(string(spa:R))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
				</td>
				<td class="td_center">
	...
	<xsl:template match="spa:SpelementUnit" mode="kpt4">
		<xsl:param name="start_number"/>
		<xsl:if test="position() &gt; $start_number and position() &lt; ($start_number + ($max_page_records*2))+1">
			<tr>
				<td class="td_center">
				...
				<td class="td_center">
					<xsl:value-of select="../../../../@CadastralNumber|../../../../../kpt:AccountNumber|../../../kpt:AccountNumber"/>
					<xsl:if test="not(string(../../../../@CadastralNumber|../../../../../kpt:AccountNumber|../../../kpt:AccountNumber))">
						<xsl:call-template name="procherk"/>
					</xsl:if>
					<!--xsl:choose>
						<xsl:when test="ancestor::kpt:Bound/kpt:SubjectsBoundary">
							<xsl:value-of select="concat('граница между субъектами Российской Федерации ',ancestor::kpt:Bound/kpt:SubjectsBoundary/kpt:NameNeighbours[1],' и ',ancestor::kpt:Bound/kpt:SubjectsBoundary/kpt:NameNeighbours[2])"/>
						</xsl:when>
						<xsl:when test="ancestor::kpt:Bound/kpt:MunicipalBoundary">
							<xsl:value-of select="concat('граница муниципального образования ',ancestor::kpt:Bound/kpt:MunicipalBoundary/kpt:Name)"/>
						</xsl:when>
						<xsl:when test="ancestor::kpt:Bound/kpt:InhabitedLocalityBoundary">
							<xsl:value-of select="concat('граница населенного пункта ', ancestor::kpt:Bound/kpt:InhabitedLocalityBoundary/kpt:Name)"/>
						</xsl:when>
						<xsl:when test="ancestor::kpt:Zone/kpt:TerritorialZone">
							<xsl:value-of select="concat('граница территориальной зоны ',ancestor::kpt:Zone/kpt:Description)"/>
						</xsl:when>
						<xsl:when test="ancestor::kpt:Zone/kpt:SpecialZone">
							<xsl:value-of select="concat('граница зоны с особыми условиями использования территории ',ancestor::kpt:Zone/kpt:Description)"/>
						</xsl:when>
						<xsl:when test="ancestor::kpt:SpatialData">
							<xsl:text>граница кадастрового квартала</xsl:text>
						</xsl:when>
					</xsl:choose-->
				</td>
		...
		<table class="tbl_page">
				...
				<td>
					...
					<nobr>
						<div class="small_text">
							<xsl:text>(полное наименование органа регистрации прав)</xsl:text>
						</div>
					</nobr>
					...
					<xsl:if test="$current_page=1">
						<p style="text-align: justify; font-size: larger;">На основании запроса от __________________, поступившего на рассмотрение _________________, сообщаем, что согласно записям Единого государственного реестра недвижимости:</p>
					</xsl:if>
					<br/>
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="1"/>
						<xsl:with-param name="pageAllCount" select="$kpt1listcount"/>
					</xsl:call-template>
					<br/>
					<xsl:if test="$current_number &lt; $max_page_records">
						<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
							<nobr>Общие сведения об объектах недвижимости в кадастровом квартале</nobr>
						</div>
						<br/>
					</xsl:if>
					<!--Основная таблица КПТ.1-->
					<table class="tbl_section_content">
						<tr>
							<td class="td_center" width="5%">
								<b>Номер п/п</b>
							</td>
							<td class="td_center" width="15%">
								<b>Кадастровый номер объекта недвижимости</b>
							</td>
							<td class="td_center" width="10%">
								<b>Вид объекта недвижимости</b>
							</td>
							<td class="td_center" width="20%">
								<b>Адрес</b>
							</td>
							<td class="td_center" width="10%">
								<b>Площадь или основная характеристика</b>
							</td>
							<td class="td_center" width="15%">
								<b>Категория земель</b>
							</td>
							<td class="td_center" width="15%">
								<b>Виды разрешенного использования</b>
							</td>
							<td class="td_center" width="15%">
								<b>Назначение (проектируемое назначение)</b>
							</td>
							<td class="td_center" width="10%">
								<b>Кадастровая стоимость (руб)</b>
							</td>
						</tr>
						<tr>
							<td class="td_center">
						...
						<!--Выводим объекты-->
						<xsl:apply-templates select="$objectsAll">
							<xsl:with-param name="start_number" select="$current_number"/>
						</xsl:apply-templates>
					</table>
				</td>
			</tr>
			...
			<tr>
				<td style="vertical-align: bottom;">
					<xsl:call-template name="section_sign"/>
				</td>
			</tr>
		</table>
		<!--Если есть еще невыведенные участки, начинаем следующую страницу-->
		<xsl:if test="($current_number + $max_page_records) &lt; $object_records">
			<xsl:call-template name="Page1">
				<xsl:with-param name="current_number" select="($current_number + $max_page_records)"/>
			...
			<tr>
				<td>
					...
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="2"/>
						<xsl:with-param name="pageAllCount" select="$kpt2listcount"/>
					</xsl:call-template>
					<br/>
					<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
						<nobr>План (чертеж, схема) объектов недвижимости, расположенных в кадастровом квартале</nobr>
					</div>
					<br/>
					<!--Основная таблица КПТ2-->
					<table class="tbl_section_content" style="border-top: 0px;">
						<tr>
							<td>
								<div align="center">
									<canvas id="canvas1" width="900" height="600" style="cursor: pointer;"/>
								</div>
							</td>
						</tr>
					</table>
					<table class="tbl_section_content" style="width: 100%; border-top: 0px;">
						<tr>
			...
			<tr>
				<td>
					...
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="3"/>
						<xsl:with-param name="pageAllCount" select="$kpt3listcount"/>
					</xsl:call-template>
					<br/>
					<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
						План (чертеж, схема) границ между субъектами Российской Федерации, границ муниципальных образований, границ населенных пунктов, расположенных в кадастровом квартале
					</div>
					<br/>
					<!--Основная таблица КПТ3-->
					<table class="tbl_section_content" style="border-top: 0px;">
						<tr>
							<td>
								<div align="center">
									<canvas id="canvas2" width="900" height="600" style="cursor: pointer;"/>
								</div>
							</td>
						</tr>
					</table>
			...
			<tr>
				<td>
					...
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="4"/>
						<xsl:with-param name="pageAllCount" select="$kpt4listcount"/>
					</xsl:call-template>
					<br/>
					<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
						<nobr>План (чертеж, схема) зон с особыми условиями использования территорий, расположенных в кадастровом квартале</nobr>
					</div>
					<br/>
					<!--Основная таблица КПТ4-->
					<table class="tbl_section_content" style="border-top: 0px;">
						<tr>
							<td>
								<div align="center">
									<canvas id="canvas3" width="900" height="600" style="cursor: pointer;"/>
								</div>
							</td>
						</tr>
					</table>
			...
			<tr>
				<td>
					...
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="5"/>
						<xsl:with-param name="pageAllCount" select="$kpt5listcount"/>
					</xsl:call-template>
					<br/>
					<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
						<nobr>План (чертеж, схема) территориальных зон, расположенных в кадастровом квартале</nobr>
					</div>
					<br/>
					<!--Основная таблица КПТ5-->
					<table class="tbl_section_content" style="border-top: 0px;">
						<tr>
							<td>
								<div align="center">
									<canvas id="canvas4" width="900" height="600" style="cursor: pointer;"/>
								</div>
							</td>
						</tr>
					</table>
			...
			<tr>
				<td>
					...
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="6"/>
						<xsl:with-param name="pageAllCount" select="$kpt6listcount"/>
					</xsl:call-template>
					<br/>
					<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
						План (чертеж, схема) территорий объектов культурного наследия, территорий опережающего социально-экономического развития, зон территориального развития в Российской Федерации, игорных зон, лесничеств, лесопарков, особо охраняемых природных территорий, особых экономических зон, охотничьих угодий, земельных участков, подлежащих образованию в соответствии с утвержденным проектом межевания территории, береговых линий (границы водного объекта), единиц кадастрового деления, расположенных в кадастровом квартале
					</div>
					<br/>
					<!--Основная таблица КПТ6-->
					<table class="tbl_section_content" style="border-top: 0px;">
						<tr>
							<td>
								<div align="center">
									<canvas id="canvas5" width="900" height="600" style="cursor: pointer;"/>
								</div>
							</td>
						</tr>
					</table>
	...
	<xsl:template name="Page7">
			...
			<tr>
				<td>
					...
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="7"/>
						<xsl:with-param name="pageAllCount" select="$kpt7listcount"/>
					</xsl:call-template>
					<br/>
					<xsl:if test="$current_number &lt; $max_page_records">
						<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
						Общая информация о зонах с особыми условиями использования территорий, территориальных зонах, территориях объектов культурного наследия, территориях опережающего социально-экономического развития, зонах территориального развития в Российской Федерации, об игорных зонах, о лесничествах, лесопарках, об особо охраняемых природных территориях, особых экономических зонах, охотничьих угодьях, о земельных участках, подлежащих образованию в соответствии с утвержденным проектом межевания территории, о береговых линиях (границе водного объекта), единицах кадастрового деления
						</div>
					</xsl:if>
					<br/>
					<!--Основные таблицы КПТ7-->
					<table class="tbl_section_content">
						<tr>
							<td class="td_center" width="10%">
								<b>Номер п/п</b>
							</td>
							<td class="td_center" width="30%">
								<b>Реестровый (учетный, условный) номер</b>
							</td>
							<td class="td_center" width="20%">
								<b>Дата присвоения</b>
							</td>
							<td class="td_center" width="40%">
								<b>Индивидуальное обозначение зоны или территории, тип и наименование водного объекта, наименование единицы кадастрового деления</b>
							</td>
						</tr>
						<tr>
							<td class="td_center">
						...
						<!--Выводим документы решений-->
						<xsl:apply-templates select="$decisions">
							<xsl:with-param name="start_number" select="$current_number"/>
						</xsl:apply-templates>
						<xsl:if test="not($decisions)">
							<tr>
								<td class="td_center">
									<xsl:call-template name="procherk"/>
								</td>
		...
		<xsl:if test="($current_number + ($max_page_records+1)) &lt; $decision_records">
			<!--Рекурсивный вызов шаблона-->
			<xsl:call-template name="Page7">
				<xsl:with-param name="current_number" select="($current_number + ($max_page_records+1))"/>
				<xsl:with-param name="current_page" select="$current_page+1"/>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	<!--КПТ.8-->
	<xsl:template name="Page8">
		<xsl:param name="current_number"/>
		<xsl:param name="current_page"/>
		<br/>
		<div class="page"/>
			...
			<tr>
				<td>
					...
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="8"/>
						<xsl:with-param name="pageAllCount" select="$kpt8listcount"/>
					</xsl:call-template>
					<br/>
					<xsl:if test="$current_number &lt; $max_page_records">
						<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
					...
					<br/>
					<!--Основная таблица КПТ8-->
					<table class="tbl_section_content">
						<tr>
							<td colspan="6">Сведения о характерных точках границ земельных участков:</td>
						</tr>
						<tr>
							<td class="td_center" rowspan="2" width="20%">
								<b>Кадастровый номер земельного участка</b>
							</td>
							<td class="td_center" rowspan="2" width="10%">
								<b>Номера точек</b>
							</td>
							<td class="td_center" colspan="2" width="20%">
								<b>Координаты</b>
							</td>
							<td class="td_center" rowspan="2" width="20%">
								<b>Описание закрепления на местности</b>
							</td>
							<td class="td_center" rowspan="2" width="30%">
								<b>Средняя квадратическая погрешность определения координат характерных точек границ земельного участка, м</b>
							</td>
						</tr>
						<tr>
							<td class="td_center">
						...
						<!--Выводим информацию о границах объектов-->
						<xsl:apply-templates select="$spatialsParcel" mode="kpt31">
							<xsl:with-param name="start_number" select="$current_number"/>
						</xsl:apply-templates>
						<xsl:if test="not($spatialsParcel)">
							<tr>
								<td class="td_center">
									<xsl:call-template name="procherk"/>
								</td>
		...
		<!--Если есть еще невыведенные границы земельных участков, начинаем следующую страницу-->
		<xsl:if test="($current_number + ($max_page_records*4)) &lt; $spatialParcels_records">
			<!--Рекурсивный вызов шаблона-->
			<xsl:call-template name="Page8">
				<xsl:with-param name="current_number" select="($current_number + ($max_page_records*4))"/>
				<xsl:with-param name="current_page" select="$current_page+1"/>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	<!--КПТ.9-->
	<xsl:template name="Page9">
		<xsl:param name="current_number"/>
		<xsl:param name="current_page"/>
		<br/>
		<div class="page"/>
			...
			<tr>
				<td>
					...
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="9"/>
						<xsl:with-param name="pageAllCount" select="$kpt9listcount"/>
					</xsl:call-template>
					<br/>
					<xsl:if test="$current_number &lt; $max_page_records">
						<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
							Описание местоположения границ зданий, сооружений, объектов незавершенного строительства на земельном участке
						</div>
					</xsl:if>
					<br/>
					<!--Основная таблица КПТ9-->
					<table class="tbl_section_content">
						<tr>
							<td colspan="8">Сведения о координатах характерных точек контура объекта недвижимости:</td>
						</tr>
						<tr>
							<td class="td_center" rowspan="2" width="20%">
								<b>Кадастровый номер здания, сооружения, объекта незавершенного строительства</b>
							</td>
							<td class="td_center" rowspan="2" width="10%">
								<b>Номера точек контура</b>
							</td>
							<td class="td_center" colspan="2" width="20%">
								<b>Координаты</b>
							</td>
							<td class="td_center" rowspan="2" width="15%">
								<b>Радиус (R)</b>
							</td>
							<td class="td_center" rowspan="2" width="20%">
								<b>Средняя квадратическая погрешность определения координат характерных точек контура, м</b>
							</td>
							<td class="td_center" colspan="2" width="15%">
								<b>Глубина, высота, м</b>
							</td>
						</tr>
						<tr>
							<td class="td_center">
						...
						<!--Выводим информацию о границах объектов-->
						<xsl:apply-templates select="$spatialsObjects" mode="kpt32">
							<xsl:with-param name="start_number" select="$current_number"/>
						</xsl:apply-templates>
						<xsl:if test="not($spatialsObjects)">
							<tr>
								<td class="td_center">
									<xsl:call-template name="procherk"/>
								</td>
		...
		<!--Если есть еще невыведенные границы объектов недвижимости, начинаем следующую страницу-->
		<xsl:if test="($current_number + ($max_page_records*4)) &lt; $spatialObjects_records">
			<!--Рекурсивный вызов шаблона-->
			<xsl:call-template name="Page9">
				<xsl:with-param name="current_number" select="($current_number + ($max_page_records*4))"/>
				<xsl:with-param name="current_page" select="$current_page+1"/>
			</xsl:call-template>
		</xsl:if>
	...
	<xsl:template name="Page10">
		...
		<table class="tbl_page">
			<tr>
				<td>
					<xsl:call-template name="section_title">
						<xsl:with-param name="pageNumber" select="10"/>
					</xsl:call-template>
					<br/>
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="10"/>
						<xsl:with-param name="pageAllCount" select="$kpt10listcount"/>
					</xsl:call-template>
					<br/>
					<xsl:if test="$current_number &lt; $max_page_records">
						<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
							Описание местоположения границ между субъектами Российской Федерации, границ муниципальных образований, границ населенных пунктов, расположенных в кадастровом квартале
						</div>
					</xsl:if>
					<br/>
					<!--Основные таблицы КПТ10-->
					<table class="tbl_section_content">
						<tr>
							<td colspan="8">Сведения о характерных точках границ:</td>
						</tr>
						<tr>
							<td class="td_center" rowspan="2" width="5%">
								<b>Номер п/п</b>
							</td>
							<td class="td_center" rowspan="2" width="20%">
								<b>Реестровый номер</b>
							</td>
							<td class="td_center" rowspan="2" width="10%">
								<b>Обозначение характерных точек</b>
							</td>
							<td class="td_center" colspan="2" width="20%">
								<b>Координаты</b>
							</td>
							...
							<td class="td_center" rowspan="2" width="20%">
								<b>Метод определения координат и средняя квадратическая погрешность положения характерной точки (Mt), м</b>
							</td>
							<td class="td_center" rowspan="2" width="15%">
						...
						<!--Выводим информацию о границах объектов-->
						<xsl:apply-templates select="$spatials10" mode="kpt4">
							<xsl:with-param name="start_number" select="$current_number"/>
						</xsl:apply-templates>
						<xsl:if test="not($spatials10)">
							<tr>
								<td class="td_center">
									<xsl:call-template name="procherk"/>
								</td>
		...
		<xsl:if test="($current_number + ($max_page_records*2)) &lt; $spatial_records10">
			<!--Рекурсивный вызов шаблона-->
			<xsl:call-template name="Page10">
				<xsl:with-param name="current_number" select="($current_number + ($max_page_records*2))"/>
				<xsl:with-param name="current_page" select="$current_page+1"/>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	<!--КПТ.11-->
	<xsl:template name="Page11">
		<xsl:param name="current_number"/>
		<xsl:param name="current_page"/>
		<br/>
		<div class="page"/>
		<table class="tbl_page">
			<tr>
				<td>
					<xsl:call-template name="section_title">
						<xsl:with-param name="pageNumber" select="11"/>
					</xsl:call-template>
					<br/>
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="11"/>
						<xsl:with-param name="pageAllCount" select="$kpt11listcount"/>
					</xsl:call-template>
					<br/>
					<xsl:if test="$current_number &lt; $max_page_records">
						<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
							Описание местоположения границ зон с особыми условиями использования территорий, территориальных зон, территорий объектов культурного наследия, территорий опережающего социально-экономического развития, зон территориального развития в Российской Федерации, игорных зон, лесничеств, лесопарков, особо охраняемых природных территорий, особых экономических зон, охотничьих угодий, земельных участков, подлежащих образованию в соответствии с утвержденным проектом межевания территории, береговых линий (границы водного объекта), единиц кадастрового деления
						</div>
					</xsl:if>
					<br/>
					<!--Основные таблицы КПТ11-->
					<table class="tbl_section_content">
						<tr>
							<td colspan="8">Сведения о характерных точках границ:</td>
						</tr>
						...
						<!--Выводим информацию о границах объектов-->
						<xsl:apply-templates select="$spatials" mode="kpt4">
							<xsl:with-param name="start_number" select="$current_number"/>
						</xsl:apply-templates>
						<xsl:if test="not($spatials)">
							<tr>
								<td class="td_center">
									<xsl:call-template name="procherk"/>
								</td>
		...
		<xsl:if test="($current_number + ($max_page_records*2)) &lt; $spatial_records">
			<!--Рекурсивный вызов шаблона-->
			<xsl:call-template name="Page11">
				<xsl:with-param name="current_number" select="($current_number + ($max_page_records*2))"/>
				<xsl:with-param name="current_page" select="$current_page+1"/>
			</xsl:call-template>
		</xsl:if>
	...
	<xsl:template name="Page12">
		...
		<table class="tbl_page">
			<tr>
				<td>
					<xsl:call-template name="section_title">
						<xsl:with-param name="pageNumber" select="12"/>
					</xsl:call-template>
					<br/>
					<xsl:call-template name="section_page">
						<xsl:with-param name="listNumber" select="$current_page"/>
						<xsl:with-param name="pageNumber" select="12"/>
						<xsl:with-param name="pageAllCount" select="$kpt12listcount"/>
					</xsl:call-template>
					<br/>
					<xsl:if test="$current_number &lt; $max_page_records">
						<div style="width: 100%; text-align: center; font-size: larger; font-weight: bold">
							<nobr>Сведения о пунктах опорной межевой сети</nobr>
						</div>
					</xsl:if>
					<br/>
					<!--Основная таблица КПТ12-->
					<table class="tbl_section_content">
						<tr>
							<td class="td_center" rowspan="2" width="20%">
								<b>Номер пункта опорной межевой сети на плане</b>
							</td>
							<td class="td_center" rowspan="2" width="30%">
								<b>Название и (или) номер, тип пункта опорной межевой сети</b>
							</td>
							<td class="td_center" rowspan="2" width="20%">
								<b>Класс опорной межевой сети</b>
							</td>
							<td class="td_center" colspan="2" width="30%">
								<b>Координаты</b>
							</td>
						</tr>
						<tr>
						...
						<!--Выводим пункты ОМС-->
						<xsl:apply-templates select="$oms">
							<xsl:with-param name="start_number" select="$current_number"/>
						</xsl:apply-templates>
					</table>
				</td>
		...
		<!--Если есть еще невыведенные пункты ОМС, начинаем следующую страницу-->
		<xsl:if test="($current_number + ($max_page_records*2)) &lt; $oms_records">
			<!--Рекурсивный вызов шаблона-->
			<xsl:call-template name="Page12">
				<xsl:with-param name="current_number" select="($current_number + ($max_page_records*2))"/>
				<xsl:with-param name="current_page" select="$current_page+1"/>
			</xsl:call-template>
		</xsl:if>
	...
	<!--Графическая часть-->
	<xsl:template match="kpt:EntitySpatial" mode="kpt21">
		<xsl:param name="count"/>
		<xsl:variable name="itemEntity" select="$count - count(following::kpt:EntitySpatial[parent::kpt:SpatialData|parent::kpt:Parcel|parent::kpt:Contour|parent::kpt:Building|parent::kpt:Construction|parent::kpt:ConditionalPartLinear|parent::kpt:Uncompleted]) - 1"/>
		<xsl:text>Coords1[</xsl:text>
		...
		<xsl:if test="parent::kpt:SpatialData">
			<xsl:text>B</xsl:text>
			<xsl:value-of select="ancestor::kpt:CadastralBlock/@CadastralNumber"/>
		</xsl:if>
		<xsl:if test="parent::kpt:Building|parent::kpt:Construction|parent::kpt:Uncompleted|parent::kpt:ConditionalPartLinear">
			<xsl:text>R</xsl:text>
			<xsl:value-of select="parent::kpt:Building/@CadastralNumber|parent::kpt:Construction/@CadastralNumber|parent::kpt:Uncompleted/@CadastralNumber"/>
			<xsl:if test="parent::kpt:ConditionalPartLinear">
				<xsl:value-of select="substring-before(string(parent::kpt:ConditionalPartLinear/@CadastralNumber),'(')"/>
			</xsl:if>
		</xsl:if>
		<xsl:if test="parent::kpt:Parcel">
			<xsl:text>P</xsl:text>
			<xsl:value-of select="ancestor::kpt:Parcel/@CadastralNumber"/>
		</xsl:if>
		<xsl:if test="parent::kpt:Contour">
			<xsl:text>C</xsl:text>
			<xsl:value-of select="ancestor::kpt:Parcel/@CadastralNumber"/>
			<xsl:text>(</xsl:text>
			<xsl:value-of select="ancestor::kpt:Contour/@NumberRecord"/>
			<xsl:text>)</xsl:text>
		</xsl:if>
		<xsl:text>";</xsl:text>
		<xsl:apply-templates select="descendant::spa:SpatialElement">
			<xsl:with-param name="numArray" select="1"/>
			<xsl:with-param name="itemEntity" select="$itemEntity"/>
			<xsl:with-param name="count" select="count(descendant::spa:SpatialElement)"/>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template match="kpt:EntitySpatial" mode="kpt22">
		<xsl:param name="count"/>
		<xsl:variable name="itemEntity" select="$count - count(following::kpt:EntitySpatial[ancestor::kpt:Bound]) - 1"/>
		<xsl:text>Coords2[</xsl:text>
		<xsl:value-of select="$itemEntity"/>
		<xsl:text>] = new Array();</xsl:text>
		<xsl:text>Coords2[</xsl:text>
		<xsl:value-of select="$itemEntity"/>
		...
		<xsl:value-of select="$itemEntity"/>
		<xsl:text>][0][0] = "</xsl:text>
		<xsl:if test="parent::kpt:Boundary">
			<xsl:text>B</xsl:text>
			<xsl:value-of select="ancestor::kpt:Bound/kpt:AccountNumber"/>
		</xsl:if>
		<xsl:text>";</xsl:text>
		<xsl:apply-templates select="descendant::spa:SpatialElement">
			<xsl:with-param name="numArray" select="2"/>
			<xsl:with-param name="itemEntity" select="$itemEntity"/>
			<xsl:with-param name="count" select="count(descendant::spa:SpatialElement)"/>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template match="kpt:EntitySpatial" mode="kpt23">
		<xsl:param name="count"/>
		<xsl:variable name="itemEntity" select="$count - count(following::kpt:EntitySpatial[parent::kpt:Zone[kpt:SpecialZone]]) - 1"/>
		<xsl:text>Coords3[</xsl:text>
		<xsl:value-of select="$itemEntity"/>
		<xsl:text>] = new Array();</xsl:text>
		<xsl:text>Coords3[</xsl:text>
		<xsl:value-of select="$itemEntity"/>
		...
		<xsl:value-of select="$itemEntity"/>
		<xsl:text>][0][0] = "</xsl:text>
		<xsl:if test="parent::kpt:Zone">
			<xsl:text>Z</xsl:text>
			<xsl:value-of select="ancestor::kpt:Zone/kpt:AccountNumber"/>
		</xsl:if>
		<xsl:text>";</xsl:text>
		<xsl:apply-templates select="descendant::spa:SpatialElement">
			<xsl:with-param name="numArray" select="3"/>
			<xsl:with-param name="itemEntity" select="$itemEntity"/>
			<xsl:with-param name="count" select="count(descendant::spa:SpatialElement)"/>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template match="kpt:EntitySpatial" mode="kpt24">
		<xsl:param name="count"/>
		<xsl:variable name="itemEntity" select="$count - count(following::kpt:EntitySpatial[parent::kpt:Zone[kpt:TerritorialZone]]) - 1"/>
		<xsl:text>Coords4[</xsl:text>
		<xsl:value-of select="$itemEntity"/>
		<xsl:text>] = new Array();</xsl:text>
		<xsl:text>Coords4[</xsl:text>
		<xsl:value-of select="$itemEntity"/>
		...
		<xsl:text>";</xsl:text>
		<xsl:apply-templates select="descendant::spa:SpatialElement">
			<xsl:with-param name="numArray" select="4"/>
			<xsl:with-param name="itemEntity" select="$itemEntity"/>
			<xsl:with-param name="count" select="count(descendant::spa:SpatialElement)"/>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template match="kpt:EntitySpatial" mode="kpt25">
		<xsl:param name="count"/>
		<xsl:variable name="itemEntity" select="$count - count(following::kpt:EntitySpatial[parent::kpt:SpatialData]) - 1"/>
		<xsl:text>Coords5[</xsl:text>
		<xsl:value-of select="$itemEntity"/>
		<xsl:text>] = new Array();</xsl:text>
		<xsl:text>Coords5[</xsl:text>
		<xsl:value-of select="$itemEntity"/>
		...
		<xsl:text>";</xsl:text>
		<xsl:apply-templates select="descendant::spa:SpatialElement">
			<xsl:with-param name="numArray" select="5"/>
			<xsl:with-param name="itemEntity" select="$itemEntity"/>
			<xsl:with-param name="count" select="count(descendant::spa:SpatialElement)"/>
		</xsl:apply-templates>
	</xsl:template>
	...
	<!--Корневой элемент-->
	<xsl:template match="kpt:KPT">
		<br/>
		<xsl:call-template name="Page1">
			<xsl:with-param name="current_number" select="0"/>
			<xsl:with-param name="current_page" select="1"/>
		</xsl:call-template>
		<xsl:if test="descendant::kpt:SpatialData/kpt:EntitySpatial|descendant::kpt:Parcel/descendant::kpt:EntitySpatial|descendant::kpt:ObjectsRealty/descendant::kpt:EntitySpatial">
		...
		<xsl:if test="descendant::kpt:Zone[kpt:TerritorialZone]/descendant::kpt:EntitySpatial">
			<xsl:call-template name="Page5">
				<xsl:with-param name="current_page" select="1"/>
			</xsl:call-template>
		</xsl:if>
		<!--xsl:if test="descendant::kpt:SpatialData/kpt:EntitySpatial">
			<xsl:call-template name="Page6">
				<xsl:with-param name="current_page" select="1"/>
			</xsl:call-template>
		</xsl:if-->
		<xsl:if test="$decisions">
			<xsl:call-template name="Page7">
				<xsl:with-param name="current_number" select="0"/>
				<xsl:with-param name="current_page" select="1"/>
			</xsl:call-template>
		</xsl:if>
		<xsl:if test="$spatialsParcel">
			<xsl:call-template name="Page8">
				<xsl:with-param name="current_number" select="0"/>
				<xsl:with-param name="current_page" select="1"/>
			</xsl:call-template>
		</xsl:if>
		<xsl:if test="$spatialsObjects">
			<xsl:call-template name="Page9">
				<xsl:with-param name="current_number" select="0"/>
				<xsl:with-param name="current_page" select="1"/>
			</xsl:call-template>
		</xsl:if>
		<xsl:if test="$spatials10">
			<xsl:call-template name="Page10">
				<xsl:with-param name="current_number" select="0"/>
				<xsl:with-param name="current_page" select="1"/>
			</xsl:call-template>
