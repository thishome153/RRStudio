...
namespace Traverser
{
    ...
    {        
        ...
        private void newToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            this.Project.Points.Clear();
            this.RawtreeView.Nodes.Clear();
            this.dataGridView1.DataSource = null;
            this.ListNumTxtObjects(this.Project.Points);
        ...
        private void openToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog1.ShowDialog(this) == DialogResult.OK)
            {
                ...
                TRing Layer = new TRing();
                Layer.ImportObjects(Project.Points);
                double TestArea = Layer.Area;

                this.ListNumTxtObjects(Project.Points);
                ...
                toolStripStatusLabel2.Text = "Area [" + Convert.ToString(Layer.Count) + "] " + Convert.ToString(Layer.Area);
            }
        GoodBy: ;
      }
        //----------------------------------------------------------------------------------------
        ...
        private void ListNumTxtObjects(BindingList<TPoint> Points)
        {
        ...
        private void ListNumTxtPolygons(TEntitySpatial PC)
        {
            PointList FullList = new PointList();
            
            for (int ic = 0; ic <= PC.Count - 1; ic++)
            {
                /*
                FullList.AppendPoints(PC[ic]);
                 for (int icc = 0; icc <= PC[ic].Childs.Count - 1; icc++)
                 FullList.AppendPoints(PC[ic].Childs[icc]);
*/
            }
            ListNumTxtObjects(FullList);
        ...
        private List<TRawObservation> ListObservations(int StationID)
        {
            ...
            if (this.Project.Raw.GetStation(StationID) != null)
            {
                ...
                DataGridViewTextBoxColumn BackCL = new DataGridViewTextBoxColumn();
                BackCL.DataPropertyName = "BackPoint";
                BackCL.HeaderText = "BackPoint";
                dataGridView1.Columns.Add(BackCL);
                ...
                DataGridViewTextBoxColumn BSHACL = new DataGridViewTextBoxColumn();
                BSHACL.DataPropertyName = "BSHA";
                BSHACL.HeaderText = "BSHA";
                dataGridView1.Columns.Add(BSHACL);
        ...
        private void OpenXYfile()
        {
            if (openFileDialog1.ShowDialog(this) == DialogResult.OK)
            {
                ...
                if (Path.GetExtension(openFileDialog1.FileName).Equals(".xml"))
                {
                    ...
                    try
                    {
                        Project.Points = (PointList)serializer.Deserialize(reader);
                        reader.Close();
                    }
                    catch (InvalidOperationException ex)
                    {
        ...
        {
            GetObservation();
            this.Project.Travers.GetMappings(); //Получить исходные пункты хода
            ListVertex();
            ListTraverseMapping();
        }
      ...
      /// </summary>
        ...
        private void ListVertex()
        {
            ...
            */
            ...
            DataGridViewTextBoxColumn DirAngleCL = new DataGridViewTextBoxColumn();
            DirAngleCL.DataPropertyName = "BSHA";
            DirAngleCL.HeaderText = "Отсчет на ЗТ, гр.";
            dataGridTraverseView.Columns.Add(DirAngleCL);
        ...
        private void ListTraverseMapping()
        {
            ...
            for (int i = 0; i <= this.Project.Travers.VertexList.Count - 1; i++)
            {
                if (this.Project.Travers.VertexList[i].Station.Status != 4)
                {
                    ...
                    this.Project.Points.Add(this.Project.Travers.VertexList[i].Station);
                }
            }

            PointsdataGridView.DataSource = this.Project.Points;//Включим грид с точками
        ...
        private void toolStripButton5_Click(object sender, EventArgs e)
        {
           ...
           PointList  res = PointsOnLine(this.Project.Points);
            if (res != null)
                this.Project.Points.AppendPoints(res);
        }

        private PointList PointsOnLine(PointList SourcePoints)
        {
