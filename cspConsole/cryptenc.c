...
/*!
 * \file $RCSfile: cryptenc.c,v $
 * \version $Revision: 1.2 $
 * \date $Date: 2001/12/25 15:57:52 $
 * \author $Author: pre $
 *
 * \brief Пример создания и обработки зашифрованного сообщения
 * с использованием функций непосредственно крипто-провайдера.
 */
...
static int cryptenc_encrypt (char *in_filename, char *out_filename,
    char *my_certfile, char **recipient_certfile, int recipient_cnt,
    char *OID);
static int cryptenc_decrypt (char *in_filename, char *out_filename,
    char *my_certfile);
...
int main_crypt_encrypt (int argc, char **argv) 
{
    ...
    if (encrypt) {
	ret = cryptenc_encrypt (in_filename, out_filename, my_certfile,
	    recipient_certfile, recipient_cnt, OID);
    } else if (decrypt) {
	ret = cryptenc_decrypt (in_filename, out_filename, my_certfile);
    } else {
	print_help = 1;
    }
...
/*----------------------------------------------------------------------*/
int cryptenc_encrypt (char *in_filename, char *out_filename, char *my_certfile, char **recipient_certfile, int recipient_cnt, char *OID)
{
...
int cryptenc_decrypt (char *in_filename, char *out_filename, char *my_certfile)
{
...
static volatile const char rcsid[] = "\n$Id: cryptenc.c,v 1.2 2001/12/25 15:57:52 pre Exp $";
#endif
