...
__delayLoadHelper(
    ...
    if (0 != (dli.dlp.fImportByName = ((pitd->u1.Ordinal & IMAGE_ORDINAL_FLAG) == 0))) {
        dli.dlp.szProcName = LPCSTR(((char *)pitd->u1.AddressOfData)+2);
        }
    else {
    ...
    if (__pfnDliNotifyHook) {
        if (0 != (pfnRet = ((*__pfnDliNotifyHook)(dliStartProcessing, &dli)))) {
            goto HookBypass;
            }
        }
